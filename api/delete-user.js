...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!["admin_account", "payroll_admin"].includes(callerProfile?.role)) {
      throw new Error("You do not have permission to delete accounts.");
    }
    const { userId } = await readBody(req);
    if (!userId || userId === caller.id) throw new Error("You cannot delete this account here.");
    const targetProfile = await profile(userId);
    if (!targetProfile || targetProfile.environment_id !== callerProfile.environment_id) {
      throw new Error("Account not found in your environment.");
    }
    if (callerProfile.role === "payroll_admin" && !(await canPayrollDelete(callerProfile, targetProfile))) {
      throw new Error("Payroll Admins cannot delete this account.");
    }
    await supabaseFetch(`/auth/v1/admin/users/${encodeURIComponent(userId)}`, { method: "DELETE" });
    return send(res, 200, { ok: true });
  } catch (error) {
    return send(res, 400, { error: error.message || "Could not delete account." });
  }
};
