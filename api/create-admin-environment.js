...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const existingProfile = await findProfile(caller.id);
    if (existingProfile?.environment_id) return send(res, 200, { profile: existingProfile });

    const body = await readBody(req);
    const fullName = String(body.name || caller.user_metadata?.full_name || caller.user_metadata?.name || caller.email?.split("@")[0] || "Admin Account").trim();
    const environments = await supabaseFetch("/rest/v1/admin_environments?select=*", {
      method: "POST",
    ...
    if (!environment?.id) throw new Error("Could not create Admin environment.");
    ...
    const profiles = await supabaseFetch("/rest/v1/profiles?on_conflict=id&select=*", {
      ...
      body: JSON.stringify({
        ...
        full_name: fullName,
        role: "admin_account",
        identification_number: `ADMIN-${String(caller.id).replaceAll("-", "").slice(0, 8)}`,
        position: "Admin Account",
        created_by: caller.id
      })
    });

    return send(res, 200, { profile: profiles?.[0] || null });
  } catch (error) {
    return send(res, 400, { error: error.message || "Could not create Admin environment." });
  }
};
